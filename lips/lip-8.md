...
This LIP does not address the initial phase of a sub-account discovery required for most merchant scenarios.
The process described below assumes both sides have the relevant subaddresses (particularly the biller side). 
For the discovery phase, several solutions are being contemplated, including:
  1. The buyer VASP triggers a process with pre-knowledge of the merchant subaddress. e.g., QR/deep links - where the consumers, while acting in their wallet, provide their VASP the merchant's relevant details (embedded in the QR/link in the checkout page)
  2. The merchant gets the user identifier (like Pay ID) and query the buyer side for a new subaddress for this process
For now, we will assume that one of these methods is used, and the merchant has the buyer subaddress at hand.
...
In this object, the initiator VASP declares its intent for pre-approval. Two potential motivations for this action:
  1. Save the consumer sub-account for future transactions (save_sub_account)- this will enable the initiator VASP (merchant) to charge the sub-account in the future but will require the owner to approve each transaction. When using this option, amount limits are not required (i.e., typical e-commerce use case).
  2. Save the consumer sub-account and get consent for future payments (consent) - this enables the initiator VASP (merchant) to charge the sub-account without any interaction with the owner (i.e., typical ride-sharing use case).
Also, the scope limits the FundPullPreApprovalObject to specific parameters of time and amount. This object can be changed by the initiator VASP if needed, but any change requires the target VASP to approve the scope change.
